...
const  AddProduct = () => {
  ...
  const AddP = () => {
       Axios.post("http://localhost:8089/ProductAdmin/Add",{name: name , price: price, rating: rating, ptype: ptype, qtype: qtype, description: description, imgurl: imgurl}).then((response)=>{
          console.log("Done");
          //alert(state)
        })
      }
    ...
    <>
      ...
      <div className="container">
          <br/><br/>
            ...
            <button type="submit" className="btn btn-primary" onClick={AddP}>Add Product</button>
          </form>
        </div>
    </>
  );
